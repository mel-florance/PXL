...
#include "texture.h"
 ...
 BasicMaterial::BasicMaterial(const std::string & name, Shader & shader) {
	...
	for (unsigned int i = 0; i < 8; i++)
	{
		...
		std::ostringstream ss_lightType;
		ss_lightType << "lightType[" << i << "]";
		this->getShader()->addUniform(ss_lightType.str());
...
void BasicMaterial::preUpdate(Scene & scene) {
	...
	for (unsigned int i = 0; i < 8; i++)
	{
		...
		if (i < scene->getLights().size())
		{
			...
			this->getShader()->setUniform1i(ss_lightType.str(), scene->getLights()[i]->getType());
