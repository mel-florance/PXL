...
void Viewport::init()
{
	...
	m_toolsPanel = new Window("Tools", glm::vec2(0.0f, 30.0f), glm::vec2(180.0f, this->getLayout()->getSize().y), "segoeui");
	m_toolsPanel->setOpacity(0.25f);
	m_toolsPanel->setDrawingShadow(false);
	m_toolsPanel->setExpandModeX(Widget::ExpandMode::FIXED);
	m_toolsPanel->setPositionModeX(Widget::ExpandMode::PARENT);
	m_toolsPanel->setPositionModeY(Widget::ExpandMode::FIXED);
